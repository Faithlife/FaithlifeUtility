# DateTimeOffsetUtility class

Provides methods for manipulating dates.

```csharp
public static class DateTimeOffsetUtility
```

## Public Members

| name | description |
| --- | --- |
| const [Iso8601Format](DateTimeOffsetUtility/Iso8601Format.md) | The ISO 8601 format string. |
| static [ParseIso8601](DateTimeOffsetUtility/ParseIso8601.md)(…) | Converts the specified ISO 8601 representation of a date and time to its DateTimeOffset equivalent. |
| static [ToIso8601](DateTimeOffsetUtility/ToIso8601.md)(…) | Formats the date in the standard ISO 8601 format. |
| static [TryParseIso8601](DateTimeOffsetUtility/TryParseIso8601.md)(…) | Converts the specified ISO 8601 representation of a date and time to its DateTimeOffset equivalent. (2 methods) |

## See Also

* namespace [Faithlife.Utility](../Faithlife.Utility.md)
* [DateTimeOffsetUtility.cs](https://github.com/Faithlife/FaithlifeUtility/tree/master/src/Faithlife.Utility/DateTimeOffsetUtility.cs)

<!-- DO NOT EDIT: generated by xmldocmd for Faithlife.Utility.dll -->
